<template>
	<div class="collect_body">
		<el-row>
			<el-button style="float: right;margin-bottom: 20px;">删除所有收藏</el-button>
		</el-row>
    			...
    			<div class="collect_Cards" id="collect_Cards" >
							...
							<div class="collectCards_body">
								<el-row v-for='(items,index) in objectresult[item-1]' :key="index"style="margin-bottom:8px;color: #A5A5A5;">
									...
									<el-col :span="18">
										<div class="grid-content1" >{{showCollectType(collectData[objectnum[item-1]+items-1].collection_module_type)}}</div>
									</el-col>
									<el-col :span="1">
                    ...
                    <el-tooltip  effect="light" content="点击删除该收藏" placement="top">
                         <i class="el-icon-more"  style="transform:rotate(90deg)" @click="HandleDelete()"></i>
									...
									<!-- <i class="el-icon-more" slot="reference" style="transform:rotate(90deg)" ></i> -->
								</el-row>					
							</div>
...
<script >
  ...
  import { searchCompanyOneData } from "@/network/tyy"
	import {getUserCollect} from "@/network/tyy"
	export default{
		name:  'Collectinfo',
		data(){
			return{
